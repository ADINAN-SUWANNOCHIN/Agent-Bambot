...
- User may say: ประนอมหนี้, restructure, ปรับโครงสร้าง, TDR group, กลุ่มประนอม, อยู่ระหว่างประนอม, กำลังปรับโครงสร้าง, กลุ่ม 01
...
- User may say: สังหาริมทรัพย์, ไม่ใช่สังหาริมทรัพย์, movable, immovable, TFRS collateral, ทรัพย์เคลื่อนย้ายได้, ทรัพย์เคลื่อนย้ายไม่ได้
...
- Code (not movable): `result = df[df['ประเภทหลักประกัน TFRS'] != 'สังหาริมทรัพย์'].shape[0]`
- Code (movable only): `result = df[df['ประเภทหลักประกัน TFRS'] == 'สังหาริมทรัพย์'].shape[0]`
...
### TDR debtors only (ยอมปรับโครงสร้างหนี้)
- User may say: TDR, ประนอมหนี้, ที่ยอมปรับโครงสร้าง, ที่ตกลงประนอม, ยินยอมปรับโครงสร้าง, ปรับปรุงโครงสร้างหนี้, ยอมเซ็น, ตกลงแล้ว, สมัครใจปรับ, เจรจาสำเร็จ, ลูกหนี้ร่วมมือ
...
### Debtors currently in restructuring process (อยู่ระหว่างการประนอมหนี้)
- User may say: อยู่ระหว่างการประนอมหนี้, กำลังประนอมหนี้, อยู่ระหว่างปรับโครงสร้าง, กลุ่มประนอม, อยู่ในกระบวนการประนอม, กำลังปรับ, อยู่ระหว่างเจรจา
- IMPORTANT: This means debtors IN group 01, not just TDR status. Use group filter:
- Code: `df[df['รหัสชื่อกลุ่มการดำเนินงาน'].str.contains('ประนอมหนี้', na=False)]`
- To COUNT them: `result = len(df[df['รหัสชื่อกลุ่มการดำเนินงาน'].str.contains('ประนอมหนี้', na=False)])`

---

## Collection Performance

### Debtors with zero payment (ไม่เคยจ่ายเลย)
...
- Code: `df[(df['เงินรับประเภท CASH'] == 0) & (df['เงินรับประเภท LGO1'] == 0)]`

### Debtors who have paid something (มีผลเรียกเก็บ)
...
- IMPORTANT: "ผลเรียกเก็บ" = total money received = CASH + LGO1 combined
- Code (debtors with ANY payment):
```python
result = df[(df['เงินรับประเภท CASH'] > 0) | (df['เงินรับประเภท LGO1'] > 0)]
```
- Code (count only): `result = ((df['เงินรับประเภท CASH'] > 0) | (df['เงินรับประเภท LGO1'] > 0)).sum()`
...
- Code: `result = df['เงินรับประเภท CASH'].sum()`

### Total recovery (cash + LGO1) = ผลเรียกเก็บรวม
...
- Formula: CASH + LGO1 = total received
- Code: `result = df['เงินรับประเภท CASH'].sum() + df['เงินรับประเภท LGO1'].sum()`
