...
- เงินรับประเภท CASH > 0 (actively making payments)
...
- **Accounts with cash collected:** `(df['เงินรับประเภท CASH'] > 0).sum()` / total accounts
- **Total cash collected rate:** `df['เงินรับประเภท CASH'].sum()` / `df['ภาระหนี้คงเหลือ'].sum()`
- **Low collection signal:** If majority of group 01 (TDR) accounts show zero CASH → restructuring plans may be failing
...
- Loan is classified NPL when payment is overdue > 90 days (3 months)
...
- **Stage 2** (credit deteriorated): Lifetime ECL provision
- **Stage 3** (NPL/Loss): Lifetime ECL, 100% provision on uncovered portion
